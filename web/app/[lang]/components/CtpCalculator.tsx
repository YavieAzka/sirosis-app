      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 space-y-2 sm:space-y-0">
        ...
        <label className="flex items-center space-x-2 cursor-pointer bg-white px-3 py-1.5 rounded-lg border border-gray-200 shadow-sm hover:bg-gray-50 transition-colors">
          ...
          <span className="text-xs font-bold text-red-900">
            {dict.mortality?.ctp_calc?.auto ||
              "Hitung Otomatis dari Parameter Klinis"}
          </span>
        </label>
      </div>
          ...
          {(Object.keys(CTP_OPTIONS) as CtpParamKey[]).map((key) => (
            <div key={key}>
              <label className="block text-xs font-bold text-gray-600 mb-1">
                {/* Opsional: Jika Anda mendaftarkan terjemahan label di JSON, panggil di sini: dict.mortality?.ctp_params?.[key] || CTP_PARAM_LABELS[key] */}
                {CTP_PARAM_LABELS[key]}
              </label>
              <select
                required={active}
              ...
              >
                <option value="">
                  {dict.mortality?.ctp_calc?.select || "-- Pilih --"}
                </option>
                {CTP_OPTIONS[key].map((opt) => (
                  <option key={opt.value} value={opt.value}>
                    {/* Opsional: Sama halnya dengan isi dropdown: dict.mortality?.ctp_options?.[key]?.[opt.value] || opt.label */}
                    {opt.label}
                  </option>
                ))}
              </select>
          ...
          {hasil && (
            <div className="mt-2 p-3 rounded-lg bg-red-50 border border-red-200 text-center">
              <p className="text-xs font-semibold text-gray-600">
                {dict.mortality?.ctp_calc?.total || "Total Skor:"}{" "}
                <span className="font-black text-red-900">{hasil.total}</span>{" "}
                &nbsp;→&nbsp; {dict.mortality?.ctp_calc?.class || "Kelas CTP:"}{" "}
                <span className="font-black text-red-900">{hasil.kelas}</span>
              </p>
            </div>
          )}
        </div>
