    ...
    <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm w-full mt-2">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 space-y-2 sm:space-y-0">
        <label className="text-sm font-bold text-gray-700">
          {dict.mortality?.inr?.title || "International Normalized Ratio (INR)"}
        </label>

        {/* Toggle Mode */}
        <div className="flex bg-white p-1 rounded-lg border border-gray-200 shadow-sm">
          <label
            className={`flex items-center space-x-1.5 cursor-pointer px-3 py-1.5 rounded-md transition-colors ${mode === "direct" ? "bg-gray-100" : "hover:bg-gray-50"}`}
          >
            <input
              type="radio"
              name="inrMode"
            ...
            <span className="text-[11px] font-bold text-gray-700 uppercase">
              {dict.mortality?.inr?.direct || "INPUT LANGSUNG"}
            </span>
          </label>
          <label
            className={`flex items-center space-x-1.5 cursor-pointer px-3 py-1.5 rounded-md transition-colors ${mode === "calculate" ? "bg-gray-100" : "hover:bg-gray-50"}`}
          >
            <input
              type="radio"
              name="inrMode"
            ...
            />
            <span className="text-[11px] font-bold text-gray-700 uppercase">
              {dict.mortality?.inr?.calculate || "HITUNG DARI PT"}
            </span>
          </label>
        </div>
      </div>
      ...
      {mode === "direct" && (
        <div>
          <input
            type="number"
            step="0.01"
            placeholder={dict.mortality?.inr?.placeholder || "Contoh: 1.2"}
            value={directInr}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2 p-4 bg-white rounded-xl border border-red-100 shadow-inner">
          <div>
            <label className="block text-xs font-bold text-gray-600 mb-1">
              {dict.mortality?.inr?.pt_patient || "PT Pasien (detik)"}
            </label>
            <input
              type="number"
              step="0.1"
              placeholder={
                dict.mortality?.inr?.placeholder_pt || "Contoh: 14.5"
              }
              value={ptPatient}
              onChange={(e) => setPtPatient(e.target.value)}
          ...
          <div>
            <label className="block text-xs font-bold text-gray-600 mb-1">
              {dict.mortality?.inr?.pt_normal || "PT Normal/Kontrol (detik)"}
            </label>
            <input
              type="number"
              step="0.1"
              placeholder={
                dict.mortality?.inr?.placeholder_normal || "Contoh: 12.0"
              }
              value={ptNormal}
              onChange={(e) => setPtNormal(e.target.value)}
              className="w-full border border-gray-300 bg-white rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-red-800 focus:border-red-800"
            />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-600 mb-1">
              {dict.mortality?.inr?.isi_value || "Nilai ISI Reagen"}
            </label>
            <input
              type="number"
              step="0.01"
          ...
          <div className="col-span-1 md:col-span-3 flex items-center justify-between bg-red-50 p-3 rounded-lg border border-red-200 shadow-sm mt-1">
            <div className="flex items-center gap-2 text-red-900">
              ...
              <span className="text-xs font-bold uppercase tracking-wider">
                {dict.mortality?.inr?.calc_result || "Hasil Kalkulasi INR:"}
              </span>
            </div>
