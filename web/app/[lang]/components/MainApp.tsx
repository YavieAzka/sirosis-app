...
export default function MainApp({ dict }: { dict: any }) {
  const [activeTab, setActiveTab] = useState<"mortalitas" | "los" | "dosis">(
    "mortalitas",
  );
  ...
  return (
    <div className="py-8 px-4 flex flex-col items-center animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row gap-2 mb-6 max-w-5xl w-full">
        ...
        >
          <HeartPulse
            className={`w-5 h-5 pointer-events-none transition-transform duration-300 ${activeTab === "mortalitas" ? "animate-pulse" : "group-hover:scale-125"}`}
          />
          {dict.mainApp?.tabs?.mortality || "AI Prediksi Mortalitas"}
        </button>
        ...
        >
          <Hospital
            className={`w-5 h-5 pointer-events-none transition-transform duration-300 group-hover:-translate-y-1 group-hover:scale-110`}
          />
          {dict.mainApp?.tabs?.los || "AI Prediksi Lama Rawat"}
        </button>
        ...
        >
          <Pill
            className={`w-5 h-5 pointer-events-none transition-transform duration-300 group-hover:rotate-45 group-hover:scale-110`}
          />
          {dict.mainApp?.tabs?.dose || "Rekomendasi Dosis Klinis"}
        </button>
      </div>
      ...
      <div className="max-w-5xl w-full bg-white rounded-3xl md:rounded-t-none shadow-2xl overflow-hidden border border-gray-100">
        {/* Pastikan melempar dict ke semua komponen anak */}
        {activeTab === "mortalitas" && <MortalityPredictor dict={dict} />}
        {activeTab === "los" && <LosPredictor dict={dict} />}
        {activeTab === "dosis" && <DoseRecommender dict={dict} />}
      </div>
    </div>
  );
}
