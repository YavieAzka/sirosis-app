...
export default function MortalityPredictor({ dict }: { dict: any }) {
  ...
  const [resultMort, setResultMort] = useState<PredictionResultMort | null>(
    null,
  );
  ...
  const [gfrParamsMort, setGfrParamsMort] = useState({
    scr: "",
    age: "",
    gender: "L",
  });
    ...
    if (autoGfrMort && gfrParamsMort.scr && gfrParamsMort.age) {
      ...
      if (!isNaN(scr) && !isNaN(age) && scr > 0 && age > 0) {
        setFormDataMort((prev) => ({
          ...prev,
          gfr: hitungGfrCkdEpi(scr, age, gfrParamsMort.gender as "L" | "P"),
        }));
      }
    }
  ...
  const handleSaveToDatabaseMort = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        alert("Data Mortalitas berhasil disimpan!");
    ...
    <div className="animate-fade-in">
      <div className="bg-gradient-to-r from-red-900 via-rose-900 to-red-800 p-8 text-center shadow-inner">
        <h1 className="text-3xl font-extrabold text-white tracking-tight drop-shadow-md">
          {dict.mortality?.title || "Kalkulator Prognosis Sirosis Hati"}
        </h1>
      </div>
      ...
      <form onSubmit={handlePredictMortalitas} className="p-8 md:p-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-2">
            ...
            >
              <option value={0}>
                {dict.mortality?.options?.no || "Tidak Ada (0)"}
              </option>
              <option value={1}>
                {dict.mortality?.options?.yes_sepsis || "Ya, Sepsis (1)"}
              </option>
            </select>
          </div>
          ...
          <div className="space-y-2">
            <label className="block text-sm font-bold text-gray-700">
              {dict.mortality?.encephalopathy || "Ensefalopati Hepatikum"}
            </label>
            <select
              className="w-full bg-gray-50 border border-gray-300 rounded-xl p-3.5 focus:ring-2 focus:ring-red-800"
              value={formDataMort.komp_eh}
              onChange={(e) =>
                setFormDataMort({
                  ...formDataMort,
            ...
            >
              ...
              <option value={1}>
                {dict.mortality?.options?.yes || "Ya (1)"}
              </option>
            </select>
          </div>

          <div className="col-span-1 md:col-span-2">
            {/* Jangan lupa untuk menambahkan dict juga ke komponen anak di bawah jika diperlukan */}
            <CtpCalculator
              active={autoCtpMort}
              onToggle={setAutoCtpMort}
              params={ctpParamsMort}
              onChangeParam={(key, value) =>
                setCtpParamsMort((prev) => ({ ...prev, [key]: value }))
              }
              hasil={skorCtpMort}
              dict={dict}
            >
              <div className="space-y-2 mt-4 border-t border-gray-200 pt-4">
                <label className="block text-sm font-bold text-gray-700">
                  {dict.mortality?.ctp_manual || "Skor CTP (Input Manual)"}
                </label>
                <select
          ...
          <div className="col-span-1 md:col-span-2">
            <InrInput onInrChange={handleInrChange} dict={dict} />
          </div>

          {[
            {
              id: "urea_baseline",
              label: "Urea Baseline (mg/dL)",
              ph: "Contoh: 25.5",
            },
            {
              id: "natrium_baseline",
              label: "Natrium Baseline (mEq/L)",
              ph: "Contoh: 138.0",
            },
            ...
            <div key={f.id} className="space-y-2">
              ...
              <input
                ...
                placeholder={f.ph}
                value={(formDataMort as any)[f.id]}
                onChange={(e) =>
                  setFormDataMort({ ...formDataMort, [f.id]: e.target.value })
                }
              />
            </div>
          ))}
          ...
          <div className="col-span-1 md:col-span-2 bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm mt-2">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 space-y-2 sm:space-y-0">
              <label className="text-sm font-bold text-gray-700">
                {dict.mortality?.gfr?.title || "Nilai GFR / CKD-EPI (mL/min)"}
              </label>
              <label className="flex items-center space-x-2 cursor-pointer bg-white px-3 py-1.5 rounded-lg border border-gray-200 shadow-sm hover:bg-gray-50 transition-colors">
                <input
                  ...
                  checked={autoGfrMort}
                  onChange={(e) => setAutoGfrMort(e.target.checked)}
                />
                <span className="text-xs font-bold text-red-900">
                  {dict.mortality?.gfr?.auto ||
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 p-4 bg-white rounded-xl border border-red-100 shadow-inner">
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">
                    {dict.mortality?.gfr?.scr || "Serum Kreatinin (mg/dL)"}
                  </label>
                  <input
                    type="number"
                    step="any"
                    required={autoGfrMort}
                    ...
                    placeholder={
                      dict.mortality?.gfr?.placeholder_scr || "Contoh: 1.1"
                    }
                    value={gfrParamsMort.scr}
                    onChange={(e) =>
                      setGfrParamsMort({
                        ...gfrParamsMort,
                        scr: e.target.value,
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">
                    {dict.mortality?.gfr?.age || "Usia (Tahun)"}
                  </label>
                  <input
                    type="number"
                    step="any"
                    ...
                    className="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-red-800 focus:border-red-800 bg-white"
                    placeholder={
                      dict.mortality?.gfr?.placeholder_age || "Contoh: 45"
                    }
                    value={gfrParamsMort.age}
                    onChange={(e) =>
                      setGfrParamsMort({
                        ...gfrParamsMort,
                        age: e.target.value,
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">
                    {dict.mortality?.gfr?.gender || "Jenis Kelamin"}
                  </label>
                  <select
                    className="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-red-800 focus:border-red-800 bg-white"
                    value={gfrParamsMort.gender}
                    onChange={(e) =>
                      setGfrParamsMort({
                        ...gfrParamsMort,
                  ...
                  >
                    <option value="L">
                      {dict.mortality?.gfr?.male || "Laki-laki"}
                    </option>
                    <option value="P">
                      {dict.mortality?.gfr?.female || "Perempuan"}
                    </option>
                  </select>
                </div>
              </div>
            ...
            <input
              ...
              placeholder={
                autoGfrMort
                  ? dict.mortality?.gfr?.placeholder_auto ||
                    "Otomatis terkalkulasi..."
                  : dict.mortality?.gfr?.placeholder_gfr || "Contoh: 85.5"
              }
              value={formDataMort.gfr}
              readOnly={autoGfrMort}
              onChange={(e) =>
                !autoGfrMort &&
                setFormDataMort({ ...formDataMort, gfr: e.target.value })
              }
            />
          </div>
        </div>
        <button
          type="submit"
          disabled={loadingMort}
          className="w-full mt-10 bg-red-800 hover:bg-red-900 text-white font-bold py-4 rounded-xl shadow-lg transition-all"
        >
          {loadingMort
            ? dict.mortality?.calculating || "Mengkalkulasi..."
            : dict.mortality?.btn_analyze || "Analisis Probabilitas Mortalitas"}
        </button>
      </form>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden my-8 border border-gray-100">
            <div className="bg-red-900 p-5 text-center">
              <h2 className="text-xl font-bold text-white tracking-wide">
                {dict.mortality?.modal?.title || "Laporan Keputusan AI"}
              </h2>
            </div>
            <div className="p-8">
              <div
                className={`text-center py-8 rounded-2xl mb-6 border shadow-inner ${resultMort.probability >= 0.4 ? "bg-red-50 border-red-200" : "bg-emerald-50 border-emerald-200"}`}
              >
                <p className="text-gray-600 font-bold mb-2 uppercase tracking-widest text-xs">
                  {dict.mortality?.modal?.prob_title || "Probabilitas Kematian"}
                </p>
                <p
                  className={`text-6xl font-black tracking-tight mb-2 ${resultMort.probability >= 0.4 ? "text-red-700" : "text-emerald-600"}`}
                >
                  {(resultMort.probability * 100).toFixed(1)}
                  <span className="text-4xl">%</span>
                </p>
                <span
                  className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${resultMort.predicted_class === 1 ? "bg-red-200 text-red-900" : "bg-emerald-200 text-emerald-900"}`}
                >
                  {dict.mortality?.modal?.ai_label || "AI Label:"}{" "}
                  {resultMort.predicted_class === 1
                    ? dict.mortality?.modal?.high_risk ||
                      "High Risk / Mortalitas"
                    : dict.mortality?.modal?.survive || "Survive"}
                </span>
              </div>
              ...
              >
                {showDetailsMort
                  ? dict.mortality?.modal?.hide_shap ||
                    "Sembunyikan Analisis Klinis (SHAP)"
                  : dict.mortality?.modal?.show_shap ||
                    "Lihat Detail Analisis Klinis AI"}
                <ChevronDown
              ...
              {showDetailsMort && (
                <div className="mb-6 bg-gray-50 rounded-xl border border-gray-200 animate-fade-in shadow-inner overflow-hidden">
                  <div className="bg-gray-100 px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                    <p className="font-extrabold text-gray-700 uppercase tracking-wider text-[10px]">
                      {dict.mortality?.modal?.risk_contrib ||
                        "Kontributor Risiko (Faktor Predominan)"}
                    </p>
                    <span className="text-[10px] font-mono bg-white px-1.5 py-0.5 rounded text-gray-500 border border-gray-200">
                      SHAP.TreeExplainer
                    </span>
                  </div>
                  <div className="p-4 space-y-3">
                    {resultMort.shap_contributions.map((shap, idx) => {
                      ...
                      // Asumsi: Anda juga bisa menerjemahkan FEATURE_LABELS di utils, tapi untuk sekarang kita biarkan seperti aslinya
                      const humanLabel =
                        FEATURE_LABELS[shap.feature] || shap.feature;
                      return (
                        <div
                          key={idx}
                        ...
                        >
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-gray-500 font-medium">
                              {dict.mortality?.modal?.patient_input ||
                                "Input pasien:"}{" "}
                          ...
                          <span
                            className={`px-2.5 py-1 rounded text-xs font-black min-w-[60px] text-center border ${isPositive ? "bg-red-50 text-red-700 border-red-200" : "bg-emerald-50 text-emerald-700 border-emerald-200"}`}
                          >
                            {isPositive ? "+" : "-"}
                            {impactValue}%
                          </span>
              ...
              <div className="mb-5 bg-gray-50 p-4 rounded-xl border border-gray-200">
                <label className="block text-xs font-bold text-gray-600 mb-2 uppercase tracking-wider text-center">
                  {dict.mortality?.modal?.auth_title || "Otorisasi Simpan Data"}
                </label>
                <input
                  type="password"
                  placeholder={
                    dict.mortality?.modal?.auth_placeholder ||
              ...
              <div className="flex gap-3">
                ...
                >
                  {dict.mortality?.modal?.btn_close || "Tutup"}
                </button>
                <button
                  onClick={handleSaveToDatabaseMort}
                  disabled={savingMort}
                  className="flex-1 bg-red-800 hover:bg-red-900 text-white py-3 rounded-xl font-bold shadow-md transition-all"
                >
                  {savingMort
                    ? dict.mortality?.modal?.btn_saving || "Menyimpan..."
                    : dict.mortality?.modal?.btn_save || "Simpan ke Database"}
                </button>
              </div>
            </div>
          </div>
