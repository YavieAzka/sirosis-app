...
export default function DoseRecommender({ dict }: { dict: any }) {
  ...
  const [gfrParamsDosis, setGfrParamsDosis] = useState({
    scr: "",
    age: "",
    gender: "L",
  });
  ...
  useEffect(() => {
    if (autoGfrDosis && gfrParamsDosis.scr && gfrParamsDosis.age) {
      ...
      if (!isNaN(scr) && !isNaN(age) && scr > 0 && age > 0) {
        setDosisData((prev) => ({
          ...prev,
          gfr: hitungGfrCkdEpi(scr, age, gfrParamsDosis.gender as "L" | "P"),
        }));
      }
    }
  }, [
  ...
  return (
    <div className="animate-fade-in flex flex-col md:flex-row">
      <div
        className={`p-8 md:p-10 ${dosisResult ? "md:w-1/2 border-r border-gray-100" : "w-full"}`}
      >
        <div className="mb-6">
        ...
        <form onSubmit={handlePredictDosis} className="space-y-5">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  >
                    <input
                      ...
                      disabled={
                        dosisData.obat_pilihan.spironolakton ||
                        dosisData.obat_pilihan.furosemid
                      }
                    />
              ...
              <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 flex flex-col">
                ...
                <div className="flex flex-col gap-3">
                  <label className="flex items-start space-x-3 cursor-pointer group">
                    <input
                      type="checkbox"
                      className="w-4 h-4 mt-0.5 text-emerald-700 rounded focus:ring-emerald-700 cursor-pointer"
                      checked={dosisData.obat_pilihan.propranolol}
                  ...
                  <label className="flex items-start space-x-3 cursor-pointer group">
                    <input
                      type="checkbox"
                      className="w-4 h-4 mt-0.5 text-emerald-700 rounded focus:ring-emerald-700 cursor-pointer"
                      checked={dosisData.obat_pilihan.carvedilol}
              ...
              <div className="bg-rose-50 p-4 rounded-xl border border-rose-100 col-span-1 md:col-span-2 flex flex-col">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <label className="flex items-start space-x-3 cursor-pointer group">
                    <input
                      type="checkbox"
                      className="w-4 h-4 mt-0.5 text-rose-700 rounded focus:ring-rose-700 cursor-pointer"
                      checked={dosisData.obat_pilihan.ampisilin_sulbaktam}
                  ...
                  <label className="flex items-start space-x-3 cursor-pointer group">
                    <input
                      type="checkbox"
                      className="w-4 h-4 mt-0.5 text-rose-700 rounded focus:ring-rose-700 cursor-pointer"
                      checked={dosisData.obat_pilihan.levofloxacin}
                  ...
                  <label className="flex items-start space-x-3 cursor-pointer group">
                    <input
                      type="checkbox"
                      className="w-4 h-4 mt-0.5 text-rose-700 rounded focus:ring-rose-700 cursor-pointer"
                      checked={dosisData.obat_pilihan.azitromisin}
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm">
              ...
              <input
                ...
                onChange={(e) =>
                  !autoGfrDosis &&
                  setDosisData({ ...dosisData, gfr: e.target.value })
                }
              />
            </div>
            ...
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm">
              ...
              <input
                ...
                onChange={(e) =>
                  !autoMapDosis &&
                  setDosisData({ ...dosisData, map_value: e.target.value })
                }
              />
            </div>
          </div>
          ...
          <CtpCalculator
            active={autoCtpDosis}
            onToggle={setAutoCtpDosis}
            params={ctpParamsDosis}
            onChangeParam={(key, value) =>
              setCtpParamsDosis((prev) => ({ ...prev, [key]: value }))
            }
            hasil={skorCtpDosis}
            dict={dict}
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {[
                ...
                >
                  <input
                    type="checkbox"
                    className="w-4 h-4 mt-0.5 text-red-800 rounded focus:ring-red-800 cursor-pointer"
                    checked={(dosisData as any)[tg.id] === 1}
                    onChange={(e) =>
                      setDosisData({
                        ...dosisData,
                        [tg.id]: e.target.checked ? 1 : 0,
                      })
                    }
                  />
          ...
          <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
            {dosisResult.info ? (
              ...
              Object.entries(dosisResult).map(([key, data]: [string, any]) => (
                <div
                  key={key}
                  className={`p-5 rounded-xl border ${data.status.includes("Avoid") || data.status.includes("Hindari") ? "bg-red-50 border-red-200" : data.status.includes("Reduce") || data.status.includes("Kurangi") || data.status.includes("Monitor") ? "bg-amber-50 border-amber-200" : data.status === "N/A" || data.status === "Belum Tersedia" ? "bg-gray-100 border-gray-300" : "bg-white border-emerald-200 shadow-sm"}`}
                >
